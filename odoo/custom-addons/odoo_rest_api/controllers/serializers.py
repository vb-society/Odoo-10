...
from .exceptions import QueryFormatError, raise_from_cause
...
class Serializer(object):
    ...
    def get_parsed_restql_query(self):
        ...
        try:
            ...
            raise raise_from_cause(SyntaxError(msg),None)
        except QueryFormatError as e:
            msg = "QueryFormatError: " + str(e)
            #raise QueryFormatError(msg) from None
            raise raise_from_cause(QueryFormatError(msg),None)
    ...
    def build_flat_field(cls, rec, field_name):
        ...
        if field_type in ['one2many', 'many2many']:
            ...
            return {
                field_name:  rec[field_name] if isinstance(rec[field_name], str) else rec[field_name].strftime("%Y-%m-%d-%H-%M")
            }
        elif field_type == 'date' and rec[field_name]:
            return {
                field_name: rec[field_name] if isinstance(rec[field_name], str) else rec[field_name].strftime("%Y-%m-%d")
            }
        elif field_type == 'time' and rec[field_name]:
            return {
                field_name: rec[field_name] if isinstance(rec[field_name], str) else rec[field_name].strftime("%H-%M-%S")
            }
